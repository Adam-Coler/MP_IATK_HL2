    ...
    public class Tools : Editor
{
    ...
    static void LogPhotonInfo()
    {
        Debug.Log(ColorStartGreen + "Photon Connected: " + colorEnd + ColorStartRed + Photon.Pun.PhotonNetwork.IsConnected + colorEnd);
            ...
            {
                Debug.LogFormat(GlobalVariables.green + "Master Client: {0}, Player Count: {1}, Active Scene Name: {2}, Game Version: {3}, Ping: {4}, Cloud Region: {5}, Best Region: {6}, Cammeras In Room: {7}, Ping: {8}, SerializationRate: {9}, SendRate: {10}" + GlobalVariables.endColor, PhotonNetwork.IsMasterClient.ToString(), Convert.ToInt32(PhotonNetwork.CountOfPlayersInRooms).ToString(), SceneManagerHelper.ActiveSceneName, PhotonNetwork.GameVersion, PhotonNetwork.GetPing().ToString(), PhotonNetwork.CloudRegion, PhotonNetwork.BestRegionSummaryInPreferences, (Camera.allCameras.Length.ToString()), PhotonNetwork.GetPing(), Photon.Pun.PhotonNetwork.SerializationRate, Photon.Pun.PhotonNetwork.SendRate);
