    ...
    public class AnnotationManagerSaveLoadEvents : MonoBehaviourPun
    {
        ...
        private void centerAnnotationOnSpawnPoint(Annotation annotation)
        {
            if (annotation.myAnnotationType == Annotation.typesOfAnnotations.LINERENDER) {
                Debug.LogFormat(GlobalVariables.cCommon + "Not centering annotation, is linerender.{0} {1} " + GlobalVariables.endColor + " {2}: {3} -> {4} -> {5}", "", "", Time.realtimeSinceStartup, this.gameObject.name, this.GetType(), System.Reflection.MethodBase.GetCurrentMethod());
                return; 
            } else
            {
